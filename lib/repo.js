...
Repo.prototype.getFileStat = function getFileStat(entry, fullPath, relativePath, cb) {
    exec('git log ' + path.join(relativePath, entry),
        {cwd: this.clonedir},
...
Repo.prototype.tags = function tags(cb) {
    var out,
        target = path.join(this.clonedir, '.git');

    exec('git --git-dir ' + target +  ' tag -l', function(error, stdout, stderr) {
        if(error) {
            cb(error);
        } else {
...
Repo.prototype.branches = function tags(cb) {
    ...
    var target = path.join(this.clonedir, '.git');
    exec('git --git-dir ' + target + ' branch', function(error, stdout, stderr) {
        if(error) {
            cb(error);
        } else {
            lines = stdout.split('\n');
...
Repo.prototype.checkout = function checkout(ref, cb) {
    checkoutLocalClone(this.dir, ref, cb);
};